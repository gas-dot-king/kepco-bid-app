...
function copyAiPrompt() {
  ...
  const prompt = `당신은 한전 및 발전자회사 적격심사 입찰에 정통한 '최고 수석 입찰 전략가'입니다.
아래의 몬테카를로 시뮬레이션 데이터를 심층 분석하여, 대표이사가 즉시 의사결정을 내릴 수 있는 최고 수준의 브리핑 리포트를 작성해 주십시오.

[입찰 몬테카를로 시뮬레이션 통계 데이터]
...
- 최우선 목표: 무리한 1위 싸움(출혈 경쟁)보다는, 무작위 난수가 하방으로 튀었을 때 발생하는 '사전 탈락(하한선 미달)' 리스크를 100% 차단하는 생존 우선 전략.

■ AI 분석 지시사항 (아래 목차대로 작성할 것)
1. 판세 진단: ${plantName}의 비대칭 구조(${asymLabel})가 가져오는 수학적 기댓값의 쏠림 현상을 분석할 것.
2. 최종 투찰가 추천 (1원 단위): 3가지 타겟 중 최우선 목표에 부합하는 단 하나의 금액을 볼드체로 명시할 것.
3. 추천 전략의 승리 논리: 왜 이 금액이 하한선 미달 리스크를 차단하는 완벽한 방어선인지 통계적(P-value) 근거를 들어 설명할 것.
4. 오답 노트 (리스크 경고): 추천하지 않은 나머지 2개 전략을 선택했을 때 직면하게 될 치명적인 리스크(예: 상방으로 튀었을 때의 1초 탈락 위험 등)를 경고할 것.`;
